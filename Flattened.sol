...
contract PlatziPunksDNA {
    ...
    // this pseudoRandom function is deterministic and should not be used in production
        ...
        returns(uint256)
    {
        ...
        bytes memory encodedPara = abi.encodePacked(combineParams);
        bytes32 hashedParams = keccak256(encodedPara);
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                "{",
                    ...
                    '"description": "PlatziPunks is a token host on a blockchain system",',
                    '"image":"',image,'"',
                "}"
            )
        );
